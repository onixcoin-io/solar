...
import (
	...
	"github.com/onixcoin-io/solar/contract"
	"github.com/onixcoin-io/solar/deployer"
	"github.com/onixcoin-io/solar/deployer/eth"
	"github.com/onixcoin-io/solar/deployer/onix"
	"github.com/onixcoin-io/solar/varstr"
	kingpin "gopkg.in/alecthomas/kingpin.v2"
)

var (
	app               = kingpin.New("solar", "Solidity smart contract deployment management.")
	onixRPC           = app.Flag("onix_rpc", "RPC provider url").Envar("ONIX_RPC").String()
	onixSenderAddress = app.Flag("onix_sender", "(onix) Sender UTXO Address").Envar("ONIX_SENDER").String()
...
var (
	errorUnspecifiedRPC = errors.New("Please specify RPC url by setting ONIX_RPC or ETH_RPC or using flag --onix_rpc or --eth_rpc")
)
...
func (c *solarCLI) OnixRPC() *onix.RPC {
	rpc, err := onix.NewRPC(*onixRPC)
	if err != nil {
		fmt.Println("Invalid ONIX RPC URL:", *onixRPC)
		os.Exit(1)
	}
...
func (c *solarCLI) Deployer() (deployer deployer.Deployer) {
	...
	if rawurl := *onixRPC; rawurl != "" {

		rpcURL, err = url.ParseRequestURI(rawurl)
		if err != nil {
			log.Fatalf("Invalid RPC url: %#v", rawurl)
		}
		deployer, err = onix.NewDeployer(rpcURL, c.ContractsRepository(), *onixSenderAddress)
	}
